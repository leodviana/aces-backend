...
namespace GtecIt.Controllers
{
    ...
    public class RankingController : Controller
    {
        ...
        public RankingController(IUnitOfWork uoW)
        {
            _uoW = uoW;
        ...
        public ActionResult Index(RankingIndexViewModel model, string tipoacao)
        {
            ...
            var ranking_pessoa = _uoW.ranking.ObterTodos().Where(x=>x.categoria.Equals("1")).ToList().OrderBy(x => x.pontos).ToList();
        ...
        public ActionResult Create(RankingCreateViewModel model)
        {
            ...
            var contrato = _uoW.Orcamentos.ObterTodos().Where(x => x.status.Equals("0") && x.id_Grltpatendimento == 3 && x.grlcliente.grlbasic.sexo==3)
                                .ToList()
            ...
            var professor = _uoW.Dentistas.ObterTodos().Where(x=>x.Idgrlbasic.sexo==3 && x.Ativo.Equals("S"))
                                .ToList();
        ...
        public ActionResult Create(RankingCreateViewModel model, FormCollection form)
        {
            ...
            model.categoria = "1";
            var ja_existe = _uoW.ranking.ObterTodos().Where(x => x.id_grlbasico == model.id_grlbasico && x.categoria.Equals("1")).FirstOrDefault(); 
            if (ja_existe==null)
            {
        ...
        {
            var ranking = _uoW.ranking.ObterTodos().Where(x => x.categoria == "1").OrderByDescending(x => x.pontos).ToList();
            var contador = 1;
