...
namespace GtecIt.Controllers
{
    ...
    public class RankingFemininoController : Controller
    {
        private readonly IUnitOfWork _uoW;

        public RankingFemininoController(IUnitOfWork uoW)
        {
            _uoW = uoW;
        ...
        public ActionResult Index(RankingIndexViewModel model, string tipoacao)
        {
            ...
            var ranking_pessoa = _uoW.ranking.ObterTodos().Where(x => x.categoria.Equals("2")).ToList().OrderBy(x => x.pontos).ToList();
            //var ranking_pessoa = _uoW.ranking.ObterTodos().whe.ToList().OrderBy(x => x.pontos).ToList();
            foreach (var item in ranking_pessoa)
            {
                var nome_recuperado = _uoW.Pessoas.ObterPorId(item.id_grlbasico).nome;
                lista_nome.Add(new RankingGridViewModel { id_grlbasico = item.id_grlbasico, nome = nome_recuperado, pontos = item.pontos, categoria = item.categoria, id_ranking = item.id_ranking, posicao = item.posicao });
            }
            if (tipoacao != null)
        ...
        public ActionResult Create(RankingCreateViewModel model)
        {
            ...
            var contrato = _uoW.Orcamentos.ObterTodos().Where(x => x.status.Equals("0") && x.id_Grltpatendimento == 3 && x.grlcliente.grlbasic.sexo==4)
                                .ToList()
            ...
            var professor = _uoW.Dentistas.ObterTodos().Where(x=>x.Idgrlbasic.sexo==4 && x.Ativo.Equals("S"))
                                .ToList();
        ...
        public ActionResult Create(RankingCreateViewModel model, FormCollection form)
        {
            ...
            model.categoria = "2";
            var ja_existe = _uoW.ranking.ObterTodos().Where(x => x.id_grlbasico == model.id_grlbasico && x.categoria.Equals("2")).FirstOrDefault(); 
            if (ja_existe==null)
            {
        ...
        {
            var ranking = _uoW.ranking.ObterTodos().Where(x => x.categoria == "2").OrderByDescending(x => x.pontos).ToList();
            var contador = 1;
